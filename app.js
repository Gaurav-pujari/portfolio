  ...
  projects: [
    {
      ...
      github: "https://github.com/Gaurav-pujari/marketing-analytics-dashboard",
      desc: [
  ...
  education: [
    {
      ...
      institute: "Sri Balaji University, Pune",
      year: "04/2025",
      score: "CGPA: 8.56",
    },
    {
      ...
      institute: "Dr. D.Y. Patil ACS College, Pune",
      year: "04/2023",
      score: "CGPA: 9.48",
    },
  ],
...
function renderDesc(desc) {
  if (Array.isArray(desc)) {
    return `<ul class="desc-ul">${desc.map((x) => `<li>${x}</li>`).join("")}</ul>`;
  }
  return desc || "";
}

/* ================================
   Render List (Client/Project/Exp/Edu)
================================ */
function renderList(items, container, type) {
  ...
  const link = item.github?.trim();

  d.innerHTML = `
    <div class="project-card" ${link ? `onclick="window.open('${link}','_blank')"` : ""}>
      ...
      ${
        link
          ? `<div class="item-meta" style="margin-top:10px;">🔗 View on GitHub</div>`
          : `<div class="item-meta" style="margin-top:10px;">(GitHub link not added)</div>`
      }
    </div>
  `;
}
...
function renderClientsPage() {
  ...
  (data.clients || []).forEach((c) => {
    ...
    d.innerHTML = `
      <div class="client-row">
        <img class="client-logo" src="${c.logo}" alt="${c.name}" />
        <div style="flex:1">
          <div class="item-title">${c.name}</div>
          <div class="item-meta">${(c.toolsUsed || []).join(" • ")}</div>
        </div>
      </div>
    `;
...
function renderProjectsPage() {
  ...
  (data.projects || []).forEach((p) => {
    ...
    // ✅ clickable card if github exists
    if (p.github) {
      d.style.cursor = "pointer";
      d.onclick = () => window.open(p.github, "_blank");
    }

    d.innerHTML = `
      <div class="item-title">${p.title}</div>
      <div class="item-meta">${(p.stack || []).join(" • ")}</div>

      <div class="item-desc">
        ${
          Array.isArray(p.desc)
            ? `<ul class="desc-ul">${p.desc
                .map((x) => `<li>${x}</li>`)
                .join("")}</ul>`
            : p.desc || ""
        }
      ...
      ${
        p.github
          ? `<div style="margin-top:12px; font-weight:800; color:var(--primary)">🔗 View on GitHub</div>`
          : `<div style="margin-top:12px; font-weight:800; color:var(--muted)">(GitHub link not added)</div>`
      }
    `;
